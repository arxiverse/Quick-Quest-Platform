...
function SwitchTheme() {
  ...
  const handleThemeChange = () => {
    themeAttribute();
    const newTheme =
      document.documentElement.getAttribute("data-theme") || "Neiravoid";
    setTheme(newTheme);
  };
  ...
  useEffect(() => {
    const handleUpdate = () => {
      setAnimationsEnabledState(
        localStorage.getItem("qqm-animations") !== "false",
      );
    };
    window.addEventListener("qqm-theme-update", handleUpdate);
    return () => window.removeEventListener("qqm-theme-update", handleUpdate);
  }, []);

  const setAnimationsEnabled = (enabled: boolean) => {
    setAnimationsEnabledState(enabled);
    localStorage.setItem("qqm-animations", enabled ? "true" : "false");
    window.dispatchEvent(new Event("qqm-theme-update"));
  };
