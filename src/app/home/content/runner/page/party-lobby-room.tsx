                    ...
                    {partyInfo.teamSlots.map((slot) => (
                       <div key={slot.id} className="flex flex-col items-center gap-2">
                          <div className={cn("size-16 rounded-full flex items-center justify-center text-xl shadow-inner", 
                                slot.state === "self" ? "bg-gradient-to-tr from-[#A046FF] to-[#38BDF8] text-white ring-4 ring-[#A046FF]/30" : 
                                slot.state === "filled" ? "bg-base-200 text-base-content/80" : 
                ...
                {runnerPartyLobbyChatMessages.map((message) =>
                  message.role === "giver" ? (
                    <div key={message.id} className="self-start max-w-[85%]">
                      <p className="text-[10px] text-base-content/50 ml-1 mb-0.5">{message.sender}</p>
