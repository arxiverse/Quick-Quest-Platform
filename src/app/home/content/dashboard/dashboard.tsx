...
function DashboardComponent() {
  ...
  const [leaderboardScope, setLeaderboardScope] = useState<DashboardLeaderboardScope>(
    dashboardLeaderboardScopes[0] ?? "Lokal",
  );
  const [subView, setSubView] = useState<DashboardSubView | null>(
    resolveInitialDashboardSubView,
  );
  ...
  useEffect(() => {
    syncDashboardSubViewStorage(subView);
  }, [subView]);

  if (subView?.view === "QuestDetail") {
    return (
      <QuestDetail
        questId={subView.payload.id}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {dashboardKpiItems.map((kpi) => {
          ...
          return animationsEnabled ? (
            <BorderGlow
              key={kpi.label}
              className="bg-base-100 w-full h-full rounded-[16px] border border-base-300 group hover:shadow-lg transition-transform hover:-translate-y-0.5"
              borderRadius={16}
              edgeSensitivity={20}
              glowIntensity={0.6}
              animated={false}
            ...
            <Surface
              key={kpi.label}
              className="p-0 border border-base-300/70 shrink-0"
            >
              <InnerKpi />
            </Surface>
          );
      ...
      <Surface className="p-4 sm:p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {dashboardSnapshotItems.map((item) => (
            <div
              key={item.label}
              className="rounded-[12px] border border-base-300/70 bg-base-100 p-4"
            >
