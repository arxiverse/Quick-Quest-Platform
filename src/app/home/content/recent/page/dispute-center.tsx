...
export function DisputeCenter({ questId, viewText, historyRows, onBack }: { questId: string; viewText: RecentViewText; historyRows: QuestHistoryRow[]; onBack: () => void }) {
         ...
         <div className="relative z-10 flex flex-col md:flex-row gap-6">
            <div className="flex-1">
               <span className="inline-flex items-center gap-1.5 rounded-[8px] bg-error/10 px-2.5 py-1 text-[10px] font-bold text-error">
                  ...
                  {viewText.disputeCenter.frozenBadge}
               </span>
               <h1 className="mt-3 text-2xl font-bold text-base-content">{viewText.disputeCenter.pageTitle}</h1>
               <p className="text-sm text-base-content/60 mt-1">Ref ID: {quest.questId} • {quest.title}</p>
                ...
                <div className="mt-6 p-4 rounded-[12px] bg-base-200 border border-base-300">
                  ...
                  <select className="select select-bordered w-full mt-2 font-medium bg-base-100">
                    {viewText.disputeCenter.reasonOptions.map((opt) => (
                      <option key={opt}>{opt}</option>
                    ))}
                  </select>
                  
                  <p className="text-xs font-bold text-base-content/70 mt-4">{viewText.disputeCenter.descriptionLabel}</p>
                  <textarea className="textarea textarea-bordered w-full mt-2 h-24 bg-base-100 focus:border-error" placeholder={viewText.disputeCenter.descriptionPlaceholder}></textarea>
                  ...
                  <div className="mt-3 flex items-center justify-center p-6 border-2 border-dashed border-base-300 rounded-[10px] bg-base-100/50 cursor-pointer hover:bg-base-200">
                     <p className="text-xs font-bold text-base-content/50">{viewText.disputeCenter.uploadLabel}</p>
                  </div>
               </div>
               ...
               <div className="mt-6 flex flex-col sm:flex-row gap-3">
                  <button className="btn h-12 flex-1 rounded-[10px] border-none bg-error text-white font-bold shadow-lg shadow-error/20 hover:bg-error/90">
                     {viewText.disputeCenter.submitButton}
                  </button>
               </div>
            </div>
            ...
            <div className="md:w-[350px]">
               <div className="p-4 rounded-[12px] bg-[#FEF3C7] border border-[#F59E0B]/30 text-[#92400E]">
                  <p className="font-bold text-sm">{viewText.disputeCenter.policyTitle}</p>
                  <ul className="list-disc pl-4 mt-2 text-xs space-y-1.5 opacity-80">
                    {viewText.disputeCenter.policyRules.map((rule, i) => (
                      <li key={i}>{rule}</li>
                    ))}
                  </ul>
               </div>
               ...
               <div className="mt-4 p-4 rounded-[12px] border border-base-300 bg-base-100">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-base-content/50">{viewText.disputeCenter.historyTitle}</p>
